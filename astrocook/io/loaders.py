...
def load_astrocook_fits(file_path: str, **kwargs) -> SpectrumDataV2:
    """
    Loader for FITS files generated by Astrocook (e.g. co-adds).
    Expects columns: 'x' (Wave), 'y' (Flux), 'dy' (Error).
    """
    try:
        # Load the table (usually extension 1)
        t = Table.read(file_path, format='fits')
        
        # 1. Identify Core Columns
        # Astrocook usually saves columns as 'x', 'y', 'dy'. 
        # We also check 'wave'/'flux' just in case of version differences.
        
        # Wavelength
        if 'x' in t.colnames: x = t['x'].data
        elif 'wave' in t.colnames: x = t['wave'].data
        else: raise ValueError("Astrocook FITS must have 'x' or 'wave' column.")
        
        # Flux
        if 'y' in t.colnames: y = t['y'].data
        elif 'flux' in t.colnames: y = t['flux'].data
        else: raise ValueError("Astrocook FITS must have 'y' or 'flux' column.")

        # Error
        if 'dy' in t.colnames: dy = t['dy'].data
        elif 'error' in t.colnames: dy = t['error'].data
        elif 'err' in t.colnames: dy = t['err'].data
        else: dy = np.zeros_like(y)

        # 2. Units
        # If it's from Astrocook V2, 'x' is typically already nm.
        # We can check the unit in the table if it exists, or assume nm.
        x_unit = au.nm
        if hasattr(t['x'], 'unit') and t['x'].unit is not None:
            # Trust the file unit if present
            if t['x'].unit == au.Angstrom:
                x = x / 10.0
                x_unit = au.nm
            elif t['x'].unit == au.nm:
                x_unit = au.nm
        ...
        # Automatically load any other columns (cont, model, mask) found in the file
        aux_cols = {}
        core_names = {'x', 'y', 'dy', 'wave', 'flux', 'err', 'error'}
        
        for col_name in t.colnames:
            if col_name not in core_names:
                data = t[col_name].data
                # Map specific known names to V2 standard roles
                role = col_name
                
                # Check for 'cont' (Continuum)
                if col_name in ['cont', 'continuum']:
                    # Special handling: if saved as flux-like, load as such
                    aux_cols['cont'] = DataColumnV2(data, y_unit, "Continuum")
                    continue
                
                # Check for 'model'
                if col_name in ['model']:
                    aux_cols['model'] = DataColumnV2(data, y_unit, "Model")
                    continue
                
                # General Aux
                aux_cols[role] = DataColumnV2(data, au.dimensionless_unscaled, col_name)
